...
def get_sample7d_fn(
    target_conf: list or tuple or np.ndarray,
    low_limit: list or tuple or np.ndarray,
    high_limit: list or tuple or np.ndarray,
    ratio_to_target=0.1,
):
    assert (
        len(low_limit) == 3 and len(high_limit) == 3
    ), "illegal size of limit, len(limit) must be 3"
...
def get_distance7d_fn():
    def distance7d_fn(q1, q2):
        ...
        diff_rot = np.array(
            [min((r1 - r2) ** 2, (r2 - r1) ** 2) for r1, r2 in zip(q1_rot, q2_rot)]
        )
...
def pairwise_link_collision_info(
    physicsClientId, body1, link1, body2, link2=-1.0, max_distance=0.0, **kwargs
):  # 10000
...
def set_pose(physicsClientId, body, pose):
    ...
    p.resetBasePositionAndOrientation(
        body, point, quat, physicsClientId=physicsClientId
    )


# TODO: simplified version
def get_collision7d_fn(
    physicsClientId,
    body,
    obstacles=[],
    attachments=[],
        ...
        for (body1, link1), (body2, link2) in check_body_link_pairs:
            if pairwise_link_collision(body1, link1, body2, link2, **kwargs):
                if diagnosis:
                    ...
                    cr = pairwise_link_collision_info(
                        physicsClientId, body1, link1, body2, link2
                    )
