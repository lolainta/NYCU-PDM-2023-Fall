...
def get_ur5_DH_params():
    ...
    dh_params = [
        ...
        {"a": 0.0, "d": 0.1093, "alpha": np.pi / 2},  # joint4
        {"a": 0.0, "d": 0.09475, "alpha": -np.pi / 2},  # joint5
...
def your_fk(DH_params: dict, q: list or tuple or np.ndarray, base_pos) -> np.ndarray:
    ...
    assert len(DH_params) == 6 and len(q) == 6, (
    ...
    A = A @ ts[-1]
    for i in range(6):
        jacobian[:3, i] = cross(ts[i][:3, 2], ts[-1][:3, 3] - ts[i][:3, 3])
        jacobian[3:, i] = ts[i][:3, 2]
    ...
    adjustment = np.asarray([[0, -1, 0], [0, 0, 0], [0, 0, -1]])
...
def score_fk(robot, testcase_files: str, visualize: bool = False):
    ...
    jacobian_score = [
        JACOBIAN_SCORE_MAX / testcase_file_num for _ in range(testcase_file_num)
    ]
    ...
    for file_id, testcase_file in enumerate(testcase_files):
        ...
        for i in range(cases_num):
            your_pose, your_jacobian = your_fk(
                dh_params, joint_poses[i], robot._base_position
            )
            gt_pose = poses[i]
        ...
        jacobian_score[file_id] = (
            0.0 if jacobian_score[file_id] < 0.0 else jacobian_score[file_id]
        )
        ...
        score_msg = (
            ...
            + "- Your Score Of Forward Kinematic : {:00.03f} / {:00.03f}, Error Count : {:4d} / {:4d}\n".format(
                fk_score[file_id],
                FK_SCORE_MAX / testcase_file_num,
                fk_error_cnt[file_id],
                cases_num,
            )
            + "- Your Score Of Jacobian Matrix   : {:00.03f} / {:00.03f}, Error Count : {:4d} / {:4d}\n".format(
                jacobian_score[file_id],
                JACOBIAN_SCORE_MAX / testcase_file_num,
                jacobian_error_cnt[file_id],
                cases_num,
            )
        )
    ...
    print(
        "- Your Total Score : {:00.03f} / {:00.03f}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="gui : whether show the window",
    )
    parser.add_argument(
        "--visualize-pose",
        "-vp",
