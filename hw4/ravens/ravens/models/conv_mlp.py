...
def compute_spatial_soft_argmax(
    x, batch_size, H=149, W=69, C=64
    ...
    posx, posy = tf.meshgrid(
        tf.linspace(-1.0, 1.0, num=H), tf.linspace(-1.0, 1.0, num=W), indexing="ij"
    )
...
class ConvMLP(tf.keras.Model):
    ...
    def call(self, x):
        ...
        """
        rgb = self.relu1(
            self.bn1(self.conv1(x[:, :, :, :3]))
        )  # only rgb channels through pre-trained
...
class DeepConvMLP:
    ...
    def __init__(self, input_shape, d_action, use_mdn):
        ...
        out0 = compute_spatial_soft_argmax(
            out0, self.batch_size, 320, 160, 16
        )  # shape (B, C, 2)
