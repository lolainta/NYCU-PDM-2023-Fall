...
class GtState2StepAgent(GtStateAgent):
    ...
    def train(self, dataset, num_iter, writer, validation_dataset):
        ...
        for i in range(num_iter):
            ...
            loss = train_step(
                self.pick_model, self.place_model, batch_obs, batch_act, loss_criterion
            )
            self.metric(loss)
...
class GtState3Step6DAgent(GtState6DAgent):
    ...
    def train(self, dataset, num_iter, writer, validation_dataset):
        ...
        for i in range(num_iter):
            ...
            loss = train_step(
                self.pick_model,
                self.place_se2_model,
                self.place_rpz_model,
                batch_obs,
                batch_act,
                loss_criterion,
            )
    ...
    def act(self, obs, info):
        ...
        obs_with_pick_place_se2 = np.hstack(
            (obs_with_pick, place_se2_prediction)
        ).astype(np.float32)
        ...
        p1_position = np.hstack((place_se2_prediction[0:2], place_rpz_prediction[2]))
        p1_rotation = utils.eulerXYZ_to_quatXYZW(
            (
