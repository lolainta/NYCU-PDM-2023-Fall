...
class Form2FitAgent:
    ...
    def act(self, obs, info):
        ...
        pick_heatmap = self.pick_model.forward(
            input_image, apply_softmax=True
        ).squeeze()
        ...
        pick_topk = np.int32(
            np.unravel_index(
                np.argsort(pick_heatmap.reshape(-1))[-k:], pick_heatmap.shape
            )
        ).T
        ...
        pick_descriptor = descriptors[0, pick_pixel[0], pick_pixel[1], :].reshape(1, -1)
