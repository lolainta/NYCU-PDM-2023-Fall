...
class GtStateAgent:
    ...
    def extract_x_y_theta(
        self, object_info, t_worldaug_world=None, preserve_theta=False
    ):
        ...
        object_theta = (
            -np.float32(utils.quatXYZW_to_eulerXYZ(object_quat_xyzw)[2])
            / self.theta_scale
        )
        return (
            np.hstack((object_xy, object_theta)).astype(np.float32),
            object_position,
            object_quat_xyzw,
        )
    ...
    def get_data_batch(self, dataset):
        ...
        for _ in range(self.batch_size):
            ...
            batch_act.append(
                self.act_to_gt_act(act, t_worldaug_world, transform_params)
            )  # this samples pick points from surface
...
class GtState6DAgent(GtStateAgent):
    ...
    def act(self, obs, gt_act, info):
        ...
        assert (
            False
        ), "this needs to have the ordering switched for act inference -- is now xytheta, rpz"  # pylint: disable=line-too-long
        act = {"camera_config": self.camera_config, "primitive": None}
    ...
    def info_to_gt_obs(self, info, t_worldaug_world=None):
        ...
        observation_vector = []
        for object_key in object_keys:
            object_xyzrpy = self.get_six_dof_object(info[object_key], t_worldaug_world)
    ...
    def act_to_gt_act(self, act, t_worldaug_world=None, transform_params=None):
        ...
        heightmap = np.zeros((320, 160))
        place_se3 = self.get_six_dof_act(
            transform_params, heightmap, act["params"]["pose0"], act["params"]["pose1"]
        )
