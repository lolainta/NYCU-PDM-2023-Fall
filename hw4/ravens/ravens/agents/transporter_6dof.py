...
class Transporter6dAgent(TransporterAgent):
    ...
    def __init__(self, name, task):
        ...
        self.attention_model = models.Attention(
            input_shape=self.input_shape, num_rotations=1, preprocess=self.preprocess
        )
    ...
    def get_six_dof(self, transform_params, heightmap, pose0, pose1, augment=True):
        ...
        p1_quat_wxyz = (p1_rotation[3], p1_rotation[0], p1_rotation[1], p1_rotation[2])
        ...
        p0_quat_wxyz = (p0_rotation[3], p0_rotation[0], p0_rotation[1], p0_rotation[2])
    ...
    def train(self, dataset, num_iter, writer, validation_dataset):
        ...
        for i in range(num_iter):
            ...
            _, p0, p0_theta, p1, p1_theta, z, roll, pitch = self.get_sample(dataset)
            ...
            loss2 = self.rpz_model.train(input_image, p0, p1, p1_theta, z, roll, pitch)
            del loss2
            ...
            with writer.as_default():
                ...
                tf.summary.scalar(
                    "z_loss", self.rpz_model.z_metric.result(), step=self.total_iter + i
                )
                tf.summary.scalar(
                    "roll_loss",
            ...
            if (self.total_iter + i) % validation_rate == 0:
                ...
                (
                    ...
                    roll,
                    pitch,
                ) = self.get_data_batch(validation_dataset, augment=False)
    ...
    def act(self, obs, info, compute_error=False, gt_act=None):
        ...
        p0_position = utils.pix_to_xyz(
            p0_pixel, heightmap, self.bounds, self.pixel_size
        )
        p1_position = utils.pix_to_xyz(
            p1_pixel, heightmap, self.bounds, self.pixel_size
        )

        p1_position = (p1_position[0], p1_position[1], z_best)

        p0_rotation = utils.eulerXYZ_to_quatXYZW((0, 0, -p0_theta))
        p1_rotation = utils.eulerXYZ_to_quatXYZW((roll_best, pitch_best, -p1_theta))
        ...
        if compute_error:
            ...
            gt_p0_theta = -np.float32(utils.quatXYZW_to_eulerXYZ(gt_p0_rotation)[2])
            gt_p1_theta = -np.float32(utils.quatXYZW_to_eulerXYZ(gt_p1_rotation)[2])
