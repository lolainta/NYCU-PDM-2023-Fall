...
class Task:
    ...
    def __init__(self, continuous=False):
        ...
        self.bounds = np.array([[0.25, 0.6], [-0.5, 0.5], [0, 0.3]])
    ...
    def is_match(self, pose0, pose1, symmetry):
        ...
        diff_z = np.float32(pose0[0][2]) - np.float32(pose1[0][2])
...
class ContinuousOracle:
    ...
    def __init__(
        ...
        planner_cls = (
            planners.PickPlacePlanner if ee == Suction else planners.PushPlanner
        )
    ...
    def act(self, obs, info):
        ...
        if not self._actions:
            ...
            if act is None:
                return
            self._actions = self._planner(
                self._env.get_ee_pose(), act["pose0"], act["pose1"]
            )
