...
class iCubHandsEnv(iCubEnv):
    ...
    def reset(self):
        self.robot_id = p.loadSDF(
            os.path.join(iCub.get_data_path(), "icub_model_with_hands.sdf"),
        ...
        if len(self._joints_to_control) is 0:
            for joint_name in self._joint_name_to_ids.keys():
                ...
                # + left arm and left hand, depending on the arm to control
                    ...
                    or joint_name in self.joint_groups["l_hand"]
                ...
                # or + right arm and right hand, depending on the arm to control
                    ...
                    or joint_name in self.joint_groups["r_hand"]
    ...
    def open_hand(self):
        ...
        if self._control_arm is "l":
            idx_fingers = [
                self._joint_name_to_ids[jn] for jn in self.joint_groups["l_hand"]
            ]
        else:
            idx_fingers = [
                self._joint_name_to_ids[jn] for jn in self.joint_groups["r_hand"]
            ]
    ...
    def check_contact_fingertips(self, obj_id):
        ...
        p0 = p.getContactPoints(
            obj_id,
            self.robot_id,
            linkIndexB=idx_fingers[tips_idxs[0]],
            physicsClientId=self._physics_client_id,
        )
        p1 = p.getContactPoints(
            obj_id,
            self.robot_id,
            linkIndexB=idx_fingers[tips_idxs[1]],
            physicsClientId=self._physics_client_id,
        )
        p2 = p.getContactPoints(
            obj_id,
            self.robot_id,
            linkIndexB=idx_fingers[tips_idxs[2]],
            physicsClientId=self._physics_client_id,
        )
        p3 = p.getContactPoints(
            obj_id,
            self.robot_id,
            linkIndexB=idx_fingers[tips_idxs[3]],
            physicsClientId=self._physics_client_id,
        )
        p4 = p.getContactPoints(
            obj_id,
            self.robot_id,
            linkIndexB=idx_fingers[tips_idxs[4]],
            physicsClientId=self._physics_client_id,
        )
