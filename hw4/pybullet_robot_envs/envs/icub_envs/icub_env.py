...
class iCubEnv:
    ...
    joint_groups = {
        "l_leg": ["l_knee", "l_ankle_pitch", "l_hip_pitch"],
    ...
    def __init__(
        ...
        self._control_arm = (
            control_arm if control_arm == "r" or control_arm == "l" else "l"
        )  # left arm by default
    ...
    def reset(self):
        ...
        self.robot_id = p.loadSDF(
            os.path.join(iCub.get_data_path(), "icub_model.sdf"),
        ...
        constr_id = p.createConstraint(
            ...
            childFramePosition=[
                p.getBasePositionAndOrientation(self.robot_id)[0][0],
        ...
        if len(self._joints_to_control) is 0:
            for joint_name in self._joint_name_to_ids.keys():
                ...
                # - right or left arm, depending on the arm to control
                    ...
                    joint_name in self.joint_groups["l_arm"]
                    and self._control_arm == "l"
                ) or (
                    joint_name in self.joint_groups["r_arm"]
    ...
    def get_joint_ranges(self):
        ...
        for joint_name in self._joint_name_to_ids.keys():
            ...
            joint_dumping.append(
                0.1
                if self._joint_name_to_ids[joint_name] in self._joints_to_control
                else 100.0
            )
    ...
    def _com_to_link_hand_frame(self):
        if self._control_arm is "r":
            com_T_link_hand = ((0.064668, -0.0056, -0.022681), (0.0, 0.0, 0.0, 1.0))
        else:
            com_T_link_hand = ((-0.064768, -0.00563, -0.02266), (0.0, 0.0, 0.0, 1.0))
    ...
    def apply_action(self, action, max_vel=-1):
        if self._use_IK:
            ...
            dx, dy, dz = action[:3]
            new_pos = [
                min(self._workspace_lim[0][1], max(self._workspace_lim[0][0], dx)),
            ...
            # otherwise, if it is defined as euler angles
                ...
                new_eu_orn = [
                    min(self._eu_lim[0][1], max(self._eu_lim[0][0], droll)),
            ...
            link_hand_pose = p.multiplyTransforms(
                new_pos, new_quat_orn, com_T_link_hand[0], com_T_link_hand[1]
            )
            ...
            idxs_to_block = [
                i
                for i, idx in enumerate(self._joint_name_to_ids.values())
                if idx in self._joints_to_block
            ]
