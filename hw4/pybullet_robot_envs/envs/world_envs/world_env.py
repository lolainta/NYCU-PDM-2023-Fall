...
class WorldEnv:
    ...
    def reset(self):
        ...
        table_info = p.getCollisionShapeData(
            self.table_id, -1, physicsClientId=self._physics_client_id
        )[0]
    ...
    def load_object(self, obj_name):
        ...
        self.obj_id = p.loadURDF(
            os.path.join(pybullet_data.getDataPath(), obj_name + ".urdf"),
            basePosition=self._obj_init_pose[:3],
            baseOrientation=self._obj_init_pose[3:7],
    ...
    def set_obj_pose(self, new_pos, new_quat):
        p.resetBasePositionAndOrientation(
            self.obj_id, new_pos, new_quat, physicsClientId=self._physics_client_id
        )
    ...
    def get_object_shape_info(self):
        info = list(
            p.getCollisionShapeData(
                self.obj_id, -1, physicsClientId=self._physics_client_id
            )[0]
        )
        info[4] = p.getVisualShapeData(
            self.obj_id, -1, physicsClientId=self._physics_client_id
        )[0][4]
        return info
    ...
    def check_contact(self, body_id, obj_id=None):
        ...
        pts = p.getContactPoints(
            obj_id, body_id, physicsClientId=self._physics_client_id
        )
        ...
        if self._obj_pose_rnd_std > 0:
            ...
            noise = [
                self.np_random.uniform(
                    low=-self._obj_pose_rnd_std, high=self._obj_pose_rnd_std
                ),
                self.np_random.uniform(
                    low=-self._obj_pose_rnd_std, high=self._obj_pose_rnd_std
                ),
            ...
            quat = p.getQuaternionFromEuler(
                [0, 0, self.np_random.uniform(low=-m.pi / 4, high=m.pi / 4)]
            )
