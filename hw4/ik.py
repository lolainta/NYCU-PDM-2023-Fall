...
def your_ik(
    ...
    tmp_q = np.asarray(
        [x[0] for x in q_states][2:8]
    )  # current joint angles 6d (You only need to modify this)
...
def score_ik(robot, testcase_files: str, visualize: bool = False):
    ...
    for file_id, testcase_file in enumerate(testcase_files):
        ...
        for i in range(cases_num):
            ...
            your_joint_poses = your_ik(
                robot.robot_id, poses[i], base_pos=robot._base_position
            )
    ...
    print(
        "- Your Total Score : {:00.03f} / {:00.03f}".format(
            total_ik_score, TASK2_SCORE_MAX
        )
    )
    print(
        "===================================================================================="
...
def main(args):
    ...
    p.loadURDF(
        os.path.join(pybullet_data.getDataPath(), "table/table.urdf"), [0.9, 0.0, 0.0]
    )
