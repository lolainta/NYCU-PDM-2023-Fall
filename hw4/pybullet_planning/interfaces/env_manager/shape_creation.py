...
def create_obj(path, scale=1.0, mass=STATIC_MASS, collision=True, color=GREY):
    ...
    if geometry_type == p.GEOM_BOX:
        ...
        if filename != UNKNOWN_FILE:
            ...
            if filename.endswith(".obj"):
                ...
                vertices = [
                    np.array(scale) * np.array(vertex) for vertex in mesh.vertices
                ]
                # LOGGER.debug(f'cloned from obj inside vertices_from_data | #verts {len(vertices)}')
            else:
                mio_mesh = meshio.read(filename)
                vertices = [
                    np.array(scale) * np.array(vertex) for vertex in mio_mesh.points
                ]
            ...
            try:
                ...
                mesh_data = p.getMeshData(
                    data.objectUniqueId,
                    data.linkIndex,
                    collisionShapeIndex=data.objectUniqueId,
                ...
                raise RuntimeError(
                    "Unknown file from data {} | pybullet error {}".format(data, e)
                )
...
def visual_shape_from_data(data_list, client=None):
    ...
    if len(data_list) == 1:
        ...
        if (get_data_type(data) == p.GEOM_MESH) and (file_name == UNKNOWN_FILE):
            LOGGER.warning(
                "Visual shape creation from data fails due to no filename data stored in {}".format(
                    data
                )
            )
            return NULL_ID
        ...
        return p.createVisualShapeArray(
            shapeTypes=[data.visualGeometryType for data in data_list],
...
def collision_shape_from_data(data_list, body, link, client=None):
    ...
    if len(data_list) == 1:
        ...
        if (get_data_type(data) == p.GEOM_MESH) and (file_name == UNKNOWN_FILE):
            LOGGER.warning(
                "Collision shape creation from body #{} fails due to no filename data stored in {}".format(
                    body, link, data
                )
            )
            return NULL_ID
        ...
        for data in data_list:
            ...
            file_names.append(file_name)
            poses.append(
                multiply(get_joint_inertial_pose(body, link), get_data_pose(data))
            )
            data_half_extent = np.array(get_data_extents(data)) / 2
            half_extents[0].append(data_half_extent[0])
            half_extents[1].append(data_half_extent[1])
        return p.createCollisionShapeArray(
            shapeTypes=[data.geometry_type for data in data_list],
            radii=[get_data_radius(data) for data in data_list],
            halfExtents=half_extents,
            lengths=[get_data_height(data) for data in data_list],
            fileNames=file_names,
