...
def angle_between(vec1, vec2):
    return np.math.acos(
        np.clip(np.dot(vec1, vec2) / (get_length(vec1) * get_length(vec2)), -1.0, 1.0)
    )
...
def all_between(lower_limits, values, upper_limits):
    assert len(lower_limits) == len(values), "lower limit {}, values {}".format(
        lower_limits, values
    )
    assert len(values) == len(upper_limits), "upper limit {}, values {}".format(
        upper_limits, values
    )
