...
from pybullet_planning.interfaces.robots.dynamics import (
    get_mass,
    get_dynamics_info,
    get_local_link_pose,
)
...
def clone_body(body, links=None, collision=True, visual=True, client=None):
    ...
    if model_info is not None and model_info.path.endswith(".obj"):
        new_body = create_obj(
            model_info.path, scale=model_info.scale, collision=collision
        )
...
def get_body_collision_vertices(body):
    ...
    for body_link in body_links:
        ...
        body_vertices_from_link[body_link] = apply_affine(
            world_from_com, local_from_vertices
        )
    # * base link handling
    ...
    body_vertices_from_link[BASE_LINK] = apply_affine(
        world_from_current_pose, base_local_from_vertices
    )
...
def load_model(rel_path, pose=None, **kwargs):
    from pybullet_planning.interfaces.env_manager.simulation import (
        get_model_path,
        add_data_path,
        load_pybullet,
    )
