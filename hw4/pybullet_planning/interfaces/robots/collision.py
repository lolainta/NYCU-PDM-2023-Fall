...
def pairwise_link_collision_info(
    body1, link1, body2, link2=BASE_LINK, max_distance=MAX_DISTANCE, **kwargs
):  # 10000
...
def pairwise_link_collision(
    ...
    if distance_threshold == 0.0:
        return (
            len(pairwise_link_collision_info(body1, link1, body2, link2, max_distance))
            != 0
        )
    else:
        pb_closest_pt_output = pairwise_link_collision_info(
            body1, link1, body2, link2, max_distance
        )
...
def get_collision_fn(
    ...
    * Note: This function might be one of the most heavily used function in this suite and
    ...
    from pybullet_planning.interfaces.debug_utils.debug_utils import (
    ...
    for attached in attachments:
        ...
        for ml in moving_links:
            if (
                ml != attached.parent_link
                and ((body, ml), (attached.child, BASE_LINK))
                not in extra_disabled_collisions
                and ((attached.child, BASE_LINK), (body, ml))
                not in extra_disabled_collisions
            ):
    ...
    check_body_pairs = list(
        product(moving_bodies, obstacles)
    ...
    for body1, body2 in check_body_pairs:
        ...
        for bb_links in bb_link_pairs:
            ...
            if (
                bbll_pair not in extra_disabled_collisions
                and bbll_pair[::-1] not in extra_disabled_collisions
            ):
        ...
        if not all_between(lower_limits, q, upper_limits):
            if diagnosis:
                ...
                for i, (cr_l, cr_u) in enumerate(zip(cr[0], cr[1])):
                    if cr_l:
                        LOGGER.warning(
                            "J{}: {} < lower limit {}".format(i, q[i], lower_limits[i])
                        )
                    if cr_u:
                        LOGGER.warning(
                            "J{}: {} > upper limit {}".format(i, q[i], upper_limits[i])
                        )
            return True
        ...
        for link1, link2 in self_check_link_pairs:
            if pairwise_link_collision(body, link1, body, link2):
                if diagnosis:
                    ...
                    draw_collision_diagnosis(
                        cr, body_name_from_id=body_name_from_id, **kwargs
                    )
        ...
        for body_check_links, attached_body in attach_check_pairs:
            if any_link_pair_collision(body, body_check_links, attached_body, **kwargs):
                if diagnosis:
                    ...
                    cr = any_link_pair_collision_info(
                        body, body_check_links, attached_body, **kwargs
                    )
                    draw_collision_diagnosis(
                        cr, body_name_from_id=body_name_from_id, **kwargs
                    )
...
def get_floating_body_collision_fn(
    body, obstacles=[], attachments=[], disabled_collisions={}, **kwargs
):
    ...
    function handle
        ...
        which link is colliding to which.
    """
    from pybullet_planning.interfaces.robots.collision import (
        pairwise_collision,
    ...
    for body1, body2 in check_body_pairs:
        ...
        for bb_links in bb_link_pairs:
            bbll_pair = ((body1, bb_links[0]), (body2, bb_links[1]))
            if (
                bbll_pair not in disabled_collisions
                and bbll_pair[::-1] not in disabled_collisions
            ):
                check_body_link_pairs.append(bbll_pair)
