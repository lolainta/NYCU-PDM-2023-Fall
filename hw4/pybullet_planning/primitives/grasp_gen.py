...
def get_side_cylinder_grasps(
    ...
    assert (
        safety_margin_length <= height / 2
    ), "safety margin length must be smaller than half of the bounding cylinder's height {}".format(
        height / 2
    )
    ...
    while True:
        ...
        slide_dist = random.uniform(
            -height / 2 + safety_margin_length, height / 2 - safety_margin_length
        )
        ...
        for j in range(1 + reverse_grasp):
            ...
            object_from_gripper = multiply(
                object_from_center,
                longitude_x,
                translate_along_x_axis,
