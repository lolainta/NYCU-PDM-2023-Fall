...
def direct_path(
    start, goal, extend_fn, collision_fn, sweep_collision_fn=None, **kwargs
):
    ...
    if sweep_collision_fn is not None:
        if any(
            sweep_collision_fn(q0, q1) for q0, q1 in default_selector(get_pairs(path))
        ):
            return None
    return path
