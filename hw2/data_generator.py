...
class Generator:
    ...
    def load_scene_semantic_dict(self, scene):
        with open(
            os.path.join(self._dataset_path, scene, "habitat", "info_semantic.json"),
            "r",
        ) as f:
            return json.load(f)
    ...
    def save_color_observation(self, observation, frame_number, out_folder, scene):
        ...
        color_img.save(
            os.path.join(
                out_folder, scene + self.filename_from_frame_number(frame_number)
            )
        )
        self._last_frame = np.array(color_img)

    def save_semantic_observation(
        self, observation, frame_number, out_folder, scene_dict, scene
    ):
        if not os.path.exists(out_folder):
            os.makedirs(out_folder)
        semantic = self.fix_semantic_observation(
    ...
    def save_observations(
        self, observation, frame_number, out_folder, split_name, scene_dict, scene
    ):
        self.save_color_observation(
            observation,
            frame_number,
            os.path.join(out_folder, "images", split_name),
            scene,
        )
        self.save_semantic_observation(
            observation,
            frame_number,
            os.path.join(out_folder, "annotations", f"{split_name}"),
            scene_dict,
            scene,
        )
        self.save_depth_observation(
            observation,
            frame_number,
            os.path.join(out_folder, "depth", split_name),
            scene,
        )
    ...
    def generate(self, out_folder, split_name, frames_per_room=100):
        ...
        for scene in self._scenes:
            ...
            settings["scene"] = os.path.join(
                self._dataset_path, scene, "habitat", "mesh_semantic.ply"
            )
                ...
                for _ in range(0, frames_per_room):
                    ...
                    random_state.rotation = (
                        quat_from_angle_axis(
                            np.random.uniform(0, np.pi), np.array([0, 1, 0])
                        )
                        * quat_from_angle_axis(
...
def main():
    ...
    generator.generate(out_folder=args.output, split_name="train", frames_per_room=100)
    generator.generate(out_folder=args.output, split_name="val", frames_per_room=20)
    generator.generate(out_folder=args.output, split_name="test", frames_per_room=20)
