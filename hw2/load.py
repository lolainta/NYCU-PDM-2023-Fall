...
def save_semantic_observation(obs, out_path):
    ...
    scene = "apartment_0"
    scene_dict = load_scene_semantic_dict(scene)
    semantic = fix_semantic_observation(obs, scene_dict)
    semantic_img = Image.new("L", (semantic.shape[1], semantic.shape[0]))
    semantic_img.putdata((semantic.flatten()))
    semantic_img.save(out_path)
...
def navigateAndSee(action="", data_root="data_collection/second_floor/"):
    ...
    count += 1
    cv2.imwrite(
        data_root + f"rgb/{count}.png", transform_rgb_bgr(observations["color_sensor"])
    )
    cv2.imwrite(
        data_root + f"depth/{count}.png", transform_depth(observations["depth_sensor"])
    )
    cv2.imwrite(
        data_root + f"semantic/{count}.png",
        transform_semantic(observations["semantic_sensor"]),
    )
    save_semantic_observation(
        observations["semantic_sensor"],
        os.path.join(data_root + f"annotations/{count}.png"),
    ),
